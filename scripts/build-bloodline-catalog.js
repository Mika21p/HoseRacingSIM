...
for(const [name,trait] of [
 ['Wind in Her Hair','sustained'],['Gold Ship','sustained'],['Mejiro Dober','burst'],
 ['Manhattan Cafe','sustained'],['Curren Chan','burst'],['T.M. Opera O','sustained'],
 ['Daiichi Ruby','burst']
])mainOverrides.set(name,trait);
const batchDistance=new Map([
 ['Wind in Her Hair',2400],['Gold Ship',2600],['Mejiro Dober',2200],
 ['Manhattan Cafe',2800],['Curren Chan',1200],['T.M. Opera O',2600],
 ['Daiichi Ruby',1400]
]);
const batchGrowth=new Map([
 ['Wind in Her Hair','普迟'],['Gold Ship','普迟'],['Mejiro Dober','普早'],
 ['Manhattan Cafe','普迟'],['Curren Chan','普迟'],['T.M. Opera O','普迟'],
 ['Daiichi Ruby','普早']
]);
const batchTemplate=new Map([['Mejiro Dober','◎○△'],['T.M. Opera O','○○△'],['Daiichi Ruby','◎△△']]);
const batchSecondary=new Map([['T.M. Opera O','burst']]);
const lineageSpecs=[
...
const featured=new Set('Deep Impact|King Kamehameha|Galileo|Dubawi|Tapit|Sunday Silence|Air Groove|Cesario|Gentildonna|Urban Sea|Zenyatta|Rachel Alexandra|Wind in Her Hair|Gold Ship'.split('|'));
...
const short=new Set('Lord Kanaloa|Oasis Dream|Dark Angel|Green Desert|Invincible Spirit|Pivotal|Speightstown|Ta Wee|Gold Beauty|Goodnight Olive|Meow|Curren Chan|Daiichi Ruby'.split('|'));
const eliteMares=new Set('Urban Sea|Hasili|La Troienne|Wind in Her Hair'.split('|'));
...
for(const r of rows){const core=!!r.core;if(!core&&(usage.get(r.id)||0)<4&&!anchors.has(r.id))continue;
 ...
 const secondary=batchSecondary.get(name)||types.filter(t=>t!==primary)[hash(r.id+'secondary')%2];
 let template=batchTemplate.get(name)|| (balanced.has(name)?'○○○':featured.has(name)?'◎○△':h%100<55?'◎△△':h%100<80?'○○△':'◎○△');
 let quality=eliteMares.has(name)?95:strongMares.has(name)?85:({35:50,50:65,65:75,80:85,90:95}[r.game?.breedingBase]||65);
 let coreDistance=batchDistance.get(name)|| (short.has(name)?1200:r.game?.distance|| (primary==='sustained'?2400:surface==='dirt'?1800:primary==='burst'?1600:2000));
...
const data={version:'bloodline-catalog-game-v1',note:`${coreCount}匹核心马的粗分类游戏赋值；身份与系谱沿用原库。地区、父系模板及稳定差异是设计选择，不是史实遗传结论；未知字段的模板默认均有标记。`,roots:rows.filter(r=>r.core).map(r=>r.id),lines:[...anchors.values()],lineAssignments,assignments,nicks:pairs.map(([sireLine,broodmareSireLine])=>({sireLine,broodmareSireLine}))};
