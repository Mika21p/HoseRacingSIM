...
test('world editor reveals and edits ordinary horses, retains drafts, edits templates and purges hidden DOM on close',async t=>{
 ...
 await click('[data-action=horseView][data-view=real]',dialog);assert.match(dialog.textContent,/基础能力/);assert.match(dialog.textContent,/草地|泥地/);assert.match(dialog.textContent,/瞬发|持久|消耗/);assert.doesNotMatch(dialog.textContent,/\b(grass|dirt|burst|sustained|attrition)\b/);await click('[data-action=worldEditHorse]',dialog);
