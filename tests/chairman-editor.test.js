...
test('pedigree edits rename shared ancestors, reject invalid relationships, and never fabricate ancestor traits',()=>{
 ...
 const ancestor={id:'pure',name:'仅档案',gender:'牡马',birthYear:-40,status:'ancestor'};on.pedigrees.push(ancestor);assert.deepEqual(Object.keys(E.project(on,ancestor).real).sort(),['breedingStability','breedingStrength','decline','lastInjury']);assert.throws(()=>E.preview(on,'horse','pure',{strength:80}),/无模拟/);
});
...
test('editor transactions, checkpoint retention, reload undo, migration and snapshot v7 are complete',async()=>{
 ...
 const points=await s.query('checkpoints',current.id);assert.equal(points.rows.filter(p=>p.kind==='editor').length,5);const save=await s.exportWorld(current.id);assert.equal(save.version,10);assert.equal(s.db.version,10);s.validateSnapshot(save);assert.equal(save.records.editorRecords.length,7);await s.saveSlot(current.id,1);current=await s.loadSlot(1,true);assert.equal(current.horses[0].name,'编辑6');assert.ok(E.canUndo(current));
