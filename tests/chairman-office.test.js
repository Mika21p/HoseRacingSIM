...
test("new drafts and revisions survive export and slots and are removed by recovery boundary", async () => {
  ...
  try {
    ...
    const snapshot = await store.exportWorld(w.id); store.validateSnapshot(snapshot); assert.equal(snapshot.version, 10); assert.equal(snapshot.records.scoreDrafts.length, 1);
...
test("database v2 migration preserves old journal and normalizes rating and result indexes", async () => {
  ...
  let legacy = read("js/chairman-storage.js").replace('"awards", "scoreDrafts", "revisions", "breedingEvents", "breedingYears"', '"awards"').replace(', "pedigrees"', '').replace('indexedDB.open(DB_NAME, 10)', 'indexedDB.open(DB_NAME, 2)');
  ...
  try {
    assert.equal(store.db.version, 10); const loaded = await store.load(world.id); assert.equal(loaded.rngState, out.world.rngState);
