  ...
  ns.ChairmanOfficeUI = { create(c) {
    ...
    async function render(tab) {
      ...
      if (tab === "results") {
        ...
        c.body.innerHTML = `${UI.toolbar("结果与评分", `${result.total}场`, b("latest", "上次半月汇总") + UI.more(b("hideCancelled", "隐藏取消届次")))}${filterForm("results", p, "results")}${resultCards(result.rows)}${pagination("officeMainPage", "results", result)}`; return true;
      }
      if (tab === "boards") {
    ...
    async function resultDetail(id, offset = 0) {
      ...
      modal(`${r.name} · ${date(r.turn)}`, `<div class="cm-detail-meta"><span class="cm-badge">${e(r.race.grade)}</span> ${e(r.race.surfaceRegion)} · ${e(r.race.trackName || "")} · ${e(r.race.surface)} ${r.race.distance}米 · ${e(r.trackCondition || "")} · ${badge(r)}</div><div class="cm-table-wrap"><table class="cm-score-table"><thead><tr><th>名次</th><th>马名</th><th>上场评级</th><th>机器TF</th><th>人工赛事分</th><th>距胜马</th><th>骑手</th><th>奖金万</th></tr></thead><tbody>${result.rows.map((p) => `<tr><td>${p.retired ? "退赛" : p.rank}</td><td>${link(p.horseId, p.horseName)}</td><td>${show(p.priorEventRating)}</td><td>${show(p.tf)}</td><td>${p.retired ? "—" : `<input type="number" step="any" data-draft="${e(id)}" data-performance="${e(p.id)}" aria-label="${e(p.horseName)}赛事分" value="${e(Object.hasOwn(draft?.values || {}, p.id) ? draft.values[p.id] : p.manualRating)}">${b("saveOneScore", "保存", p.id, `data-race="${e(id)}"`)}`}</td><td>${e(p.marginLabel)}</td><td>${e(p.jockeyName)}</td><td>${p.prize}</td></tr>`).join("")}</tbody></table></div><div class="cm-actions">${offset ? b("resultPage", "上一页成绩", id, `data-offset="${offset - 50}"`) : ""}${result.more ? b("resultPage", "更多成绩", id, `data-offset="${offset + 50}"`) : ""}</div><div class="cm-sticky-actions">${b("saveRaceScores", "保存本场评分", id, 'class="cm-primary"')}${UI.more(b("resetRaceScores", "重拟评分", id) + b("raceArchive", "赛事档案", r.raceId))}</div><p class="cm-draft-state">${draft ? "已恢复草稿 · 尚未应用" : "修改自动存为草稿"}</p>`, { key: `result:${id}`, render: () => resultDetail(id, offset) });
    }
    function trend(rows) {
    ...
    async function click(action, id, el) {
      ...
      if (action === "awardView") { await prefs("layout", { ...(w.ui.layout || {}), awards: id }); c.page = 0; await c.render(); }
