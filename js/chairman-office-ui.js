(function () {
  ...
  ns.ChairmanOfficeUI = { create(c) {
    ...
    function filterForm(kind, p, type, extra) {
      ...
      return `<form data-form="officeFilter" data-kind="${kind}" class="cm-filters">${input("search", isHorse ? "搜索马名" : "搜索赛事", p.search || "")}${multi("region", "地区（可多选）", [...W.regionNames(c.world), ...(type === "board" ? ["未记录"] : [])], p)}${isHorse ?
        multi("age", "年龄", [["2", "二岁"], ["3", "三岁"], ["4+", "四岁及以上"]], p) + multi("gender", "性别", ["牡马", "牝马", "骟马"], p) :
        multi("surface", "场地", ["草地", "泥地"], p) + multi("category", "距离类别", O.categories, p) + input("distance", "具体距离（米）", p.distance || "", "number") + multi("raceClass", "格付", [["g1", "G1"], ["g2", "G2"], ["g3", "G3"], ...(["results", "records"].includes(type) ? [] : [["op", "OP"]])], p)}${type === "horses" ?
        select("status", "状态", [["active", "现役"], ["retired", "退役"], ["", "全部"]], p.status ?? "active") + select("origin", "来源", [["", "全部"], ["custom", "自建"], ["ai", "自动生成"]], p.origin || "") + select("sort", "排序", [["tf", "年度TF"], ["wtr", "年度WTR"], ["prize", "生涯奖金"]], p.sort || "tf") : ""}${["calendar", "results"].includes(type) ?
    ...
    const resultCards = (rows) => `<div class="cm-table-wrap"><table><thead><tr><th>日期</th><th>赛事</th><th>条件</th><th>参赛数</th><th>评分状态</th><th></th></tr></thead><tbody>${rows.map((r) => `<tr class="${r.raceClass === "g1" ? "cm-g1" : ""}"><td>${date(r.turn)}</td><td><span class="cm-badge">${e(r.race.grade)}</span> ${r.status === "completed" ? b("result", r.name, r.id, 'class="cm-link"') : e(r.name)}</td><td>${e(r.race.surfaceRegion)} · ${e(r.race.surface)} ${r.race.distance}米</td><td>${r.count}</td><td>${r.status === "cancelled" ? "取消" : badge(r)}${r.hidden ? " · 已隐藏" : ""}</td><td>${UI.more(b("raceArchive", "赛事档案", r.raceId) + b("hideResult", r.hidden ? "恢复显示" : "隐藏", r.id, `data-hidden="${!r.hidden}"`))}</td></tr>`).join("") || '<tr><td colspan="6">暂无结果</td></tr>'}</tbody></table></div>`;
    async function render(tab) {
      ...
      if (tab === "awards") {
        ...
        c.body.innerHTML = `${UI.toolbar(view === "draft" ? `第${years()}年颁奖名单` : "奖项回顾", null, view === "draft" ? b("finish", "封存年度并进入下一年", "", w.phase === "yearEnd" ? 'class="cm-primary"' : "disabled") : "")}${UI.tabs([["draft", "本年评选"], ["history", "历史回顾"]], view, "awardView", b)}${view === "draft" ? `<form data-form="awardStrict" class="cm-actions"><label class="cm-check"><input name="strict" type="checkbox" ${w.awardsStrict ? "checked" : ""}>仅本年对应类别G1胜马</label><button>应用</button><small>名单自动保存，年末颁发</small></form><div class="cm-award-grid">${W.AWARDS.map((a) => { const h = horse(w.awardDraft[a.id]); return `<article><strong>${e(a.name)}</strong><span>${h ? link(h.id, h.name) : "空缺"}${h && !W.awardEligible(w, h, a, w.awardsStrict) ? '<small class="cm-error">不符合当前候选范围</small>' : ""}</span>${b("chooseAward", "选择 / 评语", a.id)}</article>`; }).join("")}</div>` : `<form data-form="awardYear" class="cm-actions">${input("year", "年份", p.year || "", "number")}<button>查询</button></form><div class="cm-table-wrap"><table><thead><tr><th>年份</th><th>奖项</th><th>获奖马</th><th>评语</th><th></th></tr></thead><tbody>${history.rows.map((a) => `<tr><td>第${a.year}年</td><td>${e(a.name)}</td><td>${link(a.horseId, a.horseName)}</td><td>${e(a.comment || "—")}</td><td>${a.representative ? b("result", "代表赛事", a.representative) : ""}</td></tr>`).join("")}</tbody></table></div>${pagination("officeMainPage", "awards", history)}`}`; return true;
      }
      return false;
    }
    ...
    async function resultDetail(id, offset = 0) {
      ...
      if (!r || r.raceClass === 'op') throw new Error('普通赛在后台运行，请查看马匹简要履历。');
      ...
      const anchor=all.find(p=>p.horseId===anchorId), base=draft?.benchmarkScore??r.wtrBenchmark?.score??(anchor?.tf==null ? '' : anchor.tf-delta);
      modal(`${r.name} · ${date(r.turn)}`, `<div class="cm-detail-meta"><span class="cm-badge">${e(r.race.grade)}</span> ${e(r.race.surfaceRegion)} · ${e(r.race.trackName || '')} · ${e(r.race.surface)} ${r.race.distance}米 · ${badge(r)}</div>
      <form data-form="wtrBenchmark" data-id="${e(id)}" class="cm-actions">${select('benchmarkId','基准马',all.filter(p=>!p.retired).map(p=>[p.horseId,p.horseName]),anchorId)}${input('benchmarkScore','WTR基准分',base,'number')}<button>生成推荐</button>${b('useTfBenchmark','采用TF参考基准',id)}${b('defaultWtrDraft','生成整场默认草稿',id)}<small>本场参考：TF－${delta}</small></form>
      <div class="cm-table-wrap"><table class="cm-score-table"><thead><tr><th>名次</th><th>马名</th><th>上场TF</th><th>本场TF</th><th>推荐WTR</th><th>核准WTR</th><th>距胜马</th><th>骑手</th><th>奖金万</th></tr></thead><tbody>${result.rows.map(p=>`<tr><td>${p.retired?'退赛':p.rank}</td><td>${link(p.horseId,p.horseName)}</td><td>${show(p.priorTf)}</td><td>${show(p.tf)}</td><td>${show(draft?.recommendations?.[p.id])}</td><td>${p.retired?'—':`<input type="number" step="any" data-draft="${e(id)}" data-performance="${e(p.id)}" aria-label="${e(p.horseName)}赛事分" value="${e(Object.hasOwn(draft?.values||{},p.id)?draft.values[p.id]:p.manualRating)}"><small>${draft?.touched?.[p.id]?'已调整 · ':''}已核准 ${show(p.manualRating)}</small>${b('saveOneScore','保存',p.id,`data-race="${e(id)}"`)}`}</td><td>${e(p.marginLabel)}</td><td>${e(p.jockeyName)}</td><td>${p.prize}</td></tr>`).join('')}</tbody></table></div>
      ...
      <div class="cm-sticky-actions">${b('saveRaceScores','保存本场评分',id,'class="cm-primary"')}${UI.more(b('replaceRecommendations','重新生成全部推荐',id)+b('resetRaceScores','重拟评分',id)+b('raceArchive','赛事档案',r.raceId))}</div><p class="cm-draft-state">${draft?'已恢复草稿 · 尚未应用':'修改自动存为草稿'}</p>`,{key:`result:${id}`,render:()=>resultDetail(id,offset)});
    }
    function trend(rows) {
    ...
    async function horseDetail(id, offset = 0, view = "overview") {
      const h = horse(id); if (h?.status === "juvenile" && !["pedigree", "children"].includes(view)) view = "pedigree"; if (!h) throw new Error("马匹不存在。"); detail = { kind: "horse", id, offset, view };
      ...
      if (["pedigree", "children"].includes(view)) { html += await ns.ChairmanApp.breedingContent(id, view, offset); } else if (view === "overview") {
        ...
        const result = await c.store.scanPage("performances", c.world.id, { index: "byHorseTurn", range: window.IDBKeyRange.bound([c.world.id, id, 0], [c.world.id, id, Number.MAX_SAFE_INTEGER]), reverse: true, offset,
          filter: (r) => r.raceClass !== "op" && (honors ? r.rank === 1 && r.raceClass === "g1" : (!p.year || r.year === Number(p.year)) && (r.raceName || "").includes(p.search || "") && O.matches(p.region, r.surfaceRegion) && O.matches(p.surface, r.surface) && O.matches(p.category, W.category(r.distance)) && O.matches(p.raceClass, r.raceClass) && (!p.distance || r.distance === Number(p.distance))) });
        html += honors ? "<h3>全部G1胜利</h3>" : filterForm(key, p, "records");
        html += `<div class="cm-table-wrap"><table><thead><tr><th>日期</th><th>赛事</th><th>名次</th><th>TF</th><th>人工赛事分</th><th></th></tr></thead><tbody>${result.rows.map((r) => `<tr><td>${date(r.turn)}</td><td>${b("result", r.raceName, r.occurrenceId, 'class="cm-link"')}</td><td>${r.retired ? "退赛" : r.rank}</td><td>${show(r.tf)}</td><td>${show(r.manualRating)}</td><td>${b("result", "评分", r.occurrenceId)}</td></tr>`).join("") || '<tr><td colspan="6">暂无记录</td></tr>'}</tbody></table></div>`;
    ...
    async function archive(id, offset = 0, track = false) {
      ...
      if (!track && definition.raceClass === "op") { modal(definition.name, intro + "<p>后台普通赛，成绩计入马匹简要履历。</p>"); return; }
    ...
    async function flush() {
      ...
      const state = c.dialog.querySelector(".cm-draft-state"); if (state) state.textContent = "草稿已保存 · 尚未应用";
    ...
    async function click(action, id, el) {
      ...
      if (['useTfBenchmark','defaultWtrDraft','replaceRecommendations','confirmReplaceRecommendations'].includes(action)) {
        ...
        if(action==='replaceRecommendations') {detail.recommendFields=f;modal('重新生成全部推荐',`<p>将替换本场草稿中的手动调整。已核准评分要在再次保存后才会改变。</p>${b('confirmReplaceRecommendations','替换草稿',id)}`);return true;}
    ...
    async function submit(form) {
      ...
      else if (kind === "officeFilter") { await prefs(form.dataset.kind, p); c.page = 0; if (form.dataset.kind.startsWith("horse_")) await horseDetail(form.dataset.kind.slice(6), 0, "records"); else await c.render(); }
