  ...
  ns.ChairmanOfficeUI = { create(c) {
    ...
    function filterForm(kind, p, type, extra) {
      ...
      return `<form data-form="officeFilter" data-kind="${kind}" class="cm-filters">${input("search", isHorse ? "搜索马名" : "搜索赛事", p.search || "")}${multi("region", "地区（可多选）", [...W.regionNames(c.world), ...(type === "board" ? ["未记录"] : [])], p)}${isHorse ?
        ...
        select("status", "状态", [["active", "现役"], ["retired", "退役"], ["", "全部"]], p.status ?? "active") + select("origin", "来源", [["", "全部"], ["custom", "自建"], ["ai", "自动生成"]], p.origin || "") + select("sort", "排序", [["tf", "年度TF"], ["wtr", "年度WTR"], ["prize", "生涯奖金"]], p.sort || "tf") + select("honor", "生涯荣誉", [["", "不限"], ["central", "获得中央年度奖"], ["local", "获得地方年度奖"], ["hall", "入选中央殿堂"]], p.honor || "") : ""}${["calendar", "results"].includes(type) ?
    ...
    function previousRun(p) {
      if (!p) return '首次出赛';
      const name = p.raceClass === 'op' ? e(p.raceName) : b('result', p.raceName, p.occurrenceId, 'class="cm-link"');
    ...
    async function resultDetail(id, offset = 0) {
      ...
      modal(`${r.name} · ${date(r.turn)}`, `<div class="cm-detail-meta" aria-label="比赛摘要"><span class="cm-badge">${e(r.race.grade)}</span> ${e(r.race.surfaceRegion)} · ${e(r.race.trackName || '')} · ${e(r.race.surface)} ${r.race.distance}米 · ${badge(r)}</div>
