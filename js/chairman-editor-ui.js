(function(){
  ...
  ns.ChairmanEditorUI={create(c){
    ...
    function field(k,v){const val=read(v,k);if(['aliases','regionTags'].includes(k))return `<label>${e(labels[k])}<textarea name="${e(k)}" rows="2">${e((val||[]).join('\n'))}</textarea></label>`;
      ...
      return input(k,labels[k]||k,val??'',/^(strength|weight|distMin|coreDist|distMax|age|birthYear|breedingStrength|game.breedingBase|game.distance)$/.test(k)?'number':'text');
    }
    ...
    function editor(kind,id){allowed();capture();const raw=kind==='horse'?B().get(c.world,id):E().template(c.world,id);if(!raw)throw Error('资料不存在。');const key=`${c.world.id}:${kind}:${id}`;
      const existing=drafts.get(key);draft=existing||{key,kind,id,dirty:false,values:W().clone(raw)};if(kind==='horse'&&!existing){draft.values.age=W().ageOf(c.world,raw);draft.values.breedingStrength=raw.breeding?.strength;}
