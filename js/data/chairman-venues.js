(function () {
  ...
  const tracks=table.map(([key,sourceVenueName,regionKey,city,state])=>{
    ...
    return {key,regionKey,originalName,sourceVenueName,name:`${chineseNames[key]||originalName}赛马场`,aliases:sourceVenueName===originalName?[]:[sourceVenueName],city,state,surfaces,courseType,sourceCourse:jp&&!localJbcTrack?originalName:undefined,sourceUrl:sources[0]||'',sourceUrls:sources,checkedAt:'2026-09-22',
  ...
  const pendingReasons={
    'europe-listed-goliath-cup-stakes':'官方目录未能确认同名赛事，保留待核对，不能推断为Gold Cup。',
    'europe-listed-prix-andre-baboin':'历年举办地存在变化，本轮尚未确认常用场地。'
  };
  function catalogue(){
    return ns.RaceRegistry.all().filter(r=>(['op','g1','g2','g3'].includes(r.raceClass)||jbcRaceIds.has(r.id))&&(['美国','欧洲'].includes(r.surfaceRegion)||r.surfaceRegion==='日本'&&(tracks.some(t=>t.regionKey==='japan'&&t.sourceCourse===r.course)||jbcRaceIds.has(r.id)))).map(r=>{
      ...
      const entry={sourceId:r.id,name:r.nameZh||r.name,regionKey,sourceVersion:'project-20260921',status:'pending',reason:pendingReasons[r.id]||'尚未在官方目录中确认举办地；暂不启用。'};
      if(regionKey==='japan'){
  ...
  ns.ChairmanVenues={version:'20260921.1',regionSpecs,tracks,catalogue};
})();
