(function () {
  ...
  function create(c){
    ...
    c.dialog.addEventListener('change',event=>{if(event.target.name==='worldType'&&event.target.form?.dataset.form==='world:create'){const packs=event.target.form.querySelector('[data-reference-packs]');packs.hidden=packs.disabled=event.target.value==='blank';}});
    ...
    function wizard(type){
      ...
      modal('建立主席世界',form('create','',`<div class="cm-form-grid">${input('name','世界名称','我的国际马会')}${select('worldType','世界类型',[['reference','现实参考世界'],['blank','空白新世界']],type==='blank'?'blank':'reference')}${input('seed','随机种子（留空自动）','','number')}</div><p>空白新世界从地区、马场、赛历和马群开始筹备。现实参考世界载入所选地区；日期、距离沿用项目资料，奖金和轮换顺序为游戏设定。</p><fieldset data-reference-packs ${type==='blank'?'hidden disabled':''}><legend>现实参考资料包与初始马数</legend>${ns.ChairmanVenues.regionSpecs.map(r=>{const rows=catalogue.filter(v=>v.regionKey===r.key),n=rows.filter(v=>v.status==='confirmed').length,tracks=new Set(rows.filter(v=>v.status==='confirmed').map(v=>v.venueKey)).size;return `<div class="cm-form-grid">${check('region:'+r.key,`${r.name} · ${n}场已确认／${rows.length-n}场待核对 · ${tracks}座马场`,r.key==='japan')}${input('population:'+r.key,r.name+'初始现役',r.key==='japan'?1000:Math.max(100,Math.ceil(n*5/4)*4),'number')}${input('annualTarget:'+r.key,'年度二岁马目标（留空按初始数的四分之一）','','number')}</div>`;}).join('')}</fieldset><p>年度二岁马目标默认初始数的四分之一，可在地区管理中调整。人口建议与草泥比例都是游戏参数。</p>`));
    }
    ...
    async function render(tab){
      ...
      else if(view==='tracks')html+=b('world:track','新建马场')+table(['马场','地区','州／城市','赛道','管理'],w.tracks.map(t=>[b('world:trackArchive',t.name,t.id),e(t.region),e(t.state)+' '+e(t.city),t.configurations.map(x=>e(x.name)).join('／'),b('world:track','编辑',t.id)]));
    ...
    async function submit(f){
      ...
      if(kind==='create'){
        ...
        modal('创建预览',`<p>${e(creation.name)} · ${creation.worldType==='blank'?'空白新世界，进入后创建地区与马群':`${regionKeys.length}个地区、${rows.length}场命名赛事、${new Set(rows.map(r=>r.venueKey)).size}座马场、${Object.values(population).reduce((a,b)=>a+b,0)}匹初始现役、年度二岁马目标共${regionKeys.reduce((n,k)=>n+(annualTargets[k]??Math.ceil(population[k]/4)),0)}匹；另按需求提供条件赛／补充公开赛。`}</p><form data-form="world:createConfirm"><button>建立并保存</button></form>`);return true;
      }
